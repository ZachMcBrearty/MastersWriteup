...
%     IAF due to dynamic model ~> relate to thermal timescale
...
%     when model is susceptible to IAF
...
\begin{figure}[t]
  ...
  \caption{
    ...
    % The ``snowball'' zone after 1 au represents a sudden drop in temperature due to IAF, and follows a very different powerlaw to the first zone.
...
The model is however quite complex and does not easily yield to the methods provided, so approximations such as requiring an equilibrium state must be used which mean we achieve lower bounds for the instability.

Assume the PDE can be solved with a plane wave solution of the form
\begin{equation} \label{eq:trial_plane_wave}
    T^m_n = \xi^n \exp{i k m \Delta\lambda}
\end{equation}
where $\xi$ is a (complex) scaling factor which determines the growth characteristics of the plane wave, $n$ and $m$ are the time and space indices respectively, and $k$ is the spatial frequency of the plane wave.

Taking this plane wave solution and substituting it into \eqref{eq:1DEBCM} after being discretised, dividing by the plane wave and using the definition of $\sin(x)$ in terms of exponentials we arrive at,
...
Of course an equilibrium state will not be reached if the integration is unstable, so this analysis will give a lower bound to where the instabilities of the model should occur.

Instability occurs when the scaling factor has magnitude greater than $1$, $| \xi |^2 > 1$, leading to an instability criterion of
\begin{equation}
    g \Delta t \left( \tan^2(\lambda_m) + \frac{\sin^2(k\Delta\lambda)}{(\Delta\lambda)^2}\right) > 2,
\end{equation}
suggesting that heat capacity, diffusivity, time step, spatial step, and position on the PDE all contribute to instability.
...
We may then use this equation to find the lower bound for the system will be unstable.
We can set the time step to $\Delta t = 1$ day and $g = D / C$ with $D = 0.56$ Jm$^{-2}$ s$^{-1}$ K$^{-1}$.
...
\begin{figure}[h]
    ...
    \caption{The convergence of a series of Earth-like models with varying spatial dimension (x-axis). Top: The time in years for the system to converge to a steady temperature, Bottom: the temperature converged to.
...
The discretisation of the model at the poles is slightly different (see eqns. \eqref{eq:forward_backward} and \eqref{eq:backward_forward}).
...
This is a stricter stability criteria than for the equator by a factor of $\sqrt{2}$, which is why the poles are usually the issue causing the model to diverge to infinity when the equator is fine.
It is also true that if $\cos(k\Delta\lambda) = 1$ then the model is always stable at the poles.
However, this corresponds to a trivial constant state $k = 0$ or a non-computational separation of $0$ so is not of interest.
