...
%     0DEBCM -> not dynamic, but still useful for establishing relationships between parameters e.g. Mendez
...
%     Ice-Albedo feedback due to dynamic model ~> relate to thermal timescale
...
%     when model is susceptible to Ice-Albedo feedback
...
%     F_ref, F_gas...
%     assumption of blackbody for gas giant, use of 0DEBCM for gas giant, use of Mendez for time averaged radiation 
%   C: Tidal heating;
...
This is especially apparent on the Earth where the poles have much lower temperatures (i.e. freezing) than the equator.
...
Forms for these functions are given in Section \ref{ssec:EarthLikeModel}.
...
\subsection{Earth as a baseline}
...
First the parameters of the 1DEBCM must be validated against known data, in this the Earth ...
...
\subsection{Earth-like model} \label{ssec:EarthLikeModel}
\begin{table*}
...
NC79 take data from various sources and develop a model using Legendre polynomials for latitude variation and sine and cosine functions for time variation.
The first 3 terms of the global temperature model are given in eqn. (4) of NC79 as
\begin{equation}
...
This gives a latitude dependent model for the Earth which the Earth-like model can be fitted to.
...
\subsection{Habitability and Averaging} \label{ssec:habiAver} %% move to sec 2, rename to habitability and averaging %%
...
\section{Earth-like exoplanets} \label{sec:Exoplanets} %% move to sec 2 %%%
