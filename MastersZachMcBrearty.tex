...
In co-planar orbits the moon can be eclisped by the planet with respect to the Sun.
The amount of light being eclipsed has been quantified with a variety of methods, including an n=3 body simulation, two independent 2-body solutions run in parallel, and an analytical solution with a number of assumptions.
The n-body simulation and 2-body solutions indicated that the main contributor to the eclipsing fraction is from the moon semimajor axis, with other orbital parameters having small or negilible effects (see appx. \ref{appx:eclipsing_sizes} for more information).

The n-body simulation or 2-body solutions require much smaller timesteps (~1 hour) compared to the timestep of the EBCM (1 day).
This means that if these were run in parallel with the main EBCM that most of the computational time would be spent 
