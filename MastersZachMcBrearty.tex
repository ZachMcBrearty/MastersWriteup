...
\end{equation}
By approximating $r_\text{gas} \gg a_\text{gas}$, $\alpha$ can be solved for.
...
This relation is investigated in Fig. \ref{fig:quantitative_eclipsing_moon_semimajor_axis} where the fit uses that $\arcsin(x) = x$ for small $x$.
The value of $b = (1.69 \pm 0.02) \times 10^{-4}$ is very close to the theory value of
\begin{equation}
  \frac{R_\text{gas}}{\pi \ 1.5\times10^{11} \text{m au}^{-1}} = 1.48 \times 10^{-4} \text{au},
\end{equation}
for a planet with $R_\text{gas} = 7\times10^7 m$ with $a_\text{moon}$ in au.
...
\begin{figure}
  ...
  \caption{
    ...
    Overlaid on the data is a fit of $\epsilon = a + b / x$ with parameters $a = (-3.3 \pm 0.8) \times 10^{-3}$ and $b = (1.69 +/- 0.02) \times 10^{-4}$.
  }
