...
For example Kepler-88c was discovered indirectly due to variation in the transit of Kepler-88b.
...
The Trappist-1 system is an excellent example, where Earth-like planets were found within the habitable zone of their host star.
However they are not habitable due to being tidally-locked to the star and extreme solar winds probably stripping them of any atmosphere long ago.
In any case, the first step to finding if a planet is habitable is to first model them.
...
Forms for these functions are given in Section \ref{sec:EarthLikeModel}.
...
\begin{equation}
  f_\text{time}(q, t) = \frac{1}{2} \int_{-\pi/2}^{\pi/2} H(q, \lambda, t) \cos(\lambda)d\lambda,
...
This paper uses the discrete form of these equations in Section \ref{sec:Method} in order to both quantify when a model has reached an stable average (i.e. equilibrium) temperature and for quantifying the habitability of a model which varies in time and latitude.
Also in given in Section \ref{sec:Method} are the two habitability functions used throughout this paper, as well as a.
